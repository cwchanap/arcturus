...
- [ ] On the fresh path, wrap achievement evaluation in a best-effort try/catch so a transient D1 error after the committed batch does not cause a client retry that permanently skips achievements:
...
No backfill. Use `statement-breakpoint` markers so the test migration splitter and `wrangler d1 execute --file` apply each statement independently.

### Rollout order (production)

Worker deployment and remote migration are independent operations. Follow this order:

1. Deploy the new Worker first. It tolerates old tables still existing (dual gate).
2. Observe zero `/api/chips/update` traffic for a full cycle via `wrangler tail`.
3. Apply the destructive migration remotely: `bun run db:migrate:remote`.
4. If the Worker deploys but migration fails: Worker still functions (never reads dropped tables at runtime). Re-run migration — `IF EXISTS` makes the `DROP` idempotent; `statement-breakpoint` markers let each statement apply independently.
5. If migration tracking is non-atomic (apply succeeds, `_migrations` record fails): the script prints a manual `INSERT INTO _migrations` command — run it, then re-run to confirm "all applied."
...
- [ ] `wallet_settlement` rows are permanent tombstones under this design — no automated cleanup. If future retention pruning is needed, it must never block a settlement request and must respect the unbounded retry window.
...
```ts
const lossDelta = -Math.min(firstResume.balance, MAX_ABSOLUTE_SETTLEMENT_DELTA);
if (lossDelta !== 0) {
