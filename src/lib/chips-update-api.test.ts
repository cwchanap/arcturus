...
describe('chips update API', () => {
	...
	test('rejects invalid game type', async () => {
		...
		const request = new Request('http://test.local', {
			method: 'POST',
			body: JSON.stringify({ delta: 10, gameType: 'nonexistent' }),
		});
	...
	test('rejects server-settled games (roulette) from client-authoritative endpoint', async () => {
		resetMocks();
		const POST = createHandler();
		const request = new Request('http://test.local', {
			method: 'POST',
		...
		const response = await POST({
			request,
			locals: createLocals({ user: { id: 'user-roulette' } }),
		} as any);
		const body = await readJson(response);
		expect(response.status).toBe(400);
		expect(body.error).toBe('INVALID_GAME_TYPE');
	...
	test('returns INVALID_GAME_TYPE when limits are unexpectedly missing', async () => {
		...
		const POST = createHandler({
			hasOwn: (target: object, key: PropertyKey) => {
				if (target && typeof target === 'object' && key === 'nonexistent') {
					return true;
				}
