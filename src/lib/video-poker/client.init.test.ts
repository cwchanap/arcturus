...
describe('initVideoPokerClient — authenticated settlement', () => {
	...
	test('settlement failure shows recovery controls with retry/reset buttons', async () => {
		...
		try {
			...
			expect(document.getElementById('video-poker-retry-settlement')).not.toBeNull();
			expect(document.getElementById('video-poker-reset-settlement')).not.toBeNull();
		} finally {
			root.remove();
		}
	});
	...
	test('retry handler re-attempts settlement and adopts the result on success', async () => {
		...
		installFetch({
			settlement: () => {
				...
				return settlementCall === 1
					...
					: makeResponse(200, { balance: 2000, duplicate: false });
			},
		});
		const root = buildVideoPokerDOM({
			guestMode: false,
		...
		try {
			...
			(document.getElementById('video-poker-retry-settlement') as HTMLButtonElement).click();
			await flush(15);

			expect(recoveryContainer?.classList.contains('hidden')).toBe(true);
			const balanceEl = document.getElementById('chip-balance') as HTMLElement;
			expect(balanceEl.textContent).toBe('2,000');
		} finally {
			root.remove();
		}
	});

	test('retry handler surfaces a second failure and re-enables the buttons', async () => {
		localStorage.clear();
		installFetch({ settlement: makeResponse(500, { error: 'INTERNAL_ERROR' }) });
		const root = buildVideoPokerDOM({
			guestMode: false,
		...
		try {
			...
			const statusEl = document.getElementById('video-poker-status') as HTMLElement;
			expect(statusEl.textContent).toContain('Settlement failed again');
