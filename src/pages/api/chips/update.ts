...
export function createPostHandler(overrides: Partial<PostHandlerDeps> = {}) {
	...
	return async ({ request, locals }: Parameters<APIRoute>[0]) => {
		...
		try {
			...
			// Retention cleanup for chip_sync_receipt is now handled by the
			// Cron Trigger scheduled() handler in src/worker.ts. See
			// wrangler.toml [triggers] crons and src/server/cleanup.ts.
